...
func _process(delta):
	...
	# 2. Escaneamento e Combate Constantes
	...
	# 4. Avaliação de Utilidade (Macro Estado)
	...
	# =====================================================
	# 5. O PULSO CEREBRAL (TIME-SLICING LOD)
	# =====================================================
	_think_timer -= delta
	if _think_timer <= 0.0:
	...
	if is_instance_valid(driver):
		...
		_cached_intencoes["jump"] = false # Consome o pulo para não pular no vazio depois
...
func _evaluate_utility():
	...
	var tem_inimigo_perto = _tem_inimigo_em_raio(40000.0) # 200m
	var tem_inimigo_muito_perto = _tem_inimigo_em_raio(2500.0) # 50m
