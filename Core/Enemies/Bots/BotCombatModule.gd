...
# NOVO: Flag para avisar o cérebro que o bot conseguiu atirar no VIP
...
func processar_combate(delta: float, current_state: int, alvo_atual: Node3D):
	disparou_no_vip = false # Reseta a cada frame
...
func _gerenciar_armas_do_bot(current_state: int, alvo_atual: Node3D):
	...
	# Estado 3 é ATTACK, 4 é FLEE, 6 é SEEK_HEIGHT
	...
	if is_instance_valid(alvo_tiro):
		...
		# NOVA REGRA: Se o alvo for o VIP, ele atira mesmo se a mira não estiver 100% perfeita
...
# ... (bloco do dot_p > dot_limite_ataque) ...
		if current_state == 3 and dot_p > dot_limite_ataque:
			var active_w = wm.get_active_special()
			if active_w and wm.shooter.special_cooldowns.get(active_w.nome, 0.0) <= 0:
				if wm.shooter.try_fire_special(active_w, false):
					
					# A MÁGICA AQUI: O Módulo avisa que o tiro no VIP foi um sucesso!
					if is_vip: 
...
func _gerenciar_habilidades_do_bot(current_state: int, alvo_atual: Node3D):
	...
	if current_state == 4 or (current_state == 3 and is_instance_valid(alvo_atual) and car.global_position.distance_to(alvo_atual.global_position) > 40.0):
