...
func _update_visuals():
	# Se não tiver um .tres ou não tiver malha, aborta
	if not status_data or not mesh_instance: return
	
	if status_data.custom_mesh:
		# 1. Aplica a malha e a escala do Resource
		mesh_instance.mesh = status_data.custom_mesh
		mesh_instance.scale = status_data.mesh_scale
