...
@export var FALL_FORCE_BUFFER_DISTANCE = 2.5 
...
@export var STEER_TORQUE_START = 8.0  # Força de giro a 25 km/h
@export var STEER_TORQUE_END = 2.0    # Força de giro a 300 km/h
...
func _handle_engine_and_steering(delta, is_on_ground, speed_mps):
	...
	var speed_kmh = speed_mps * 2.0 # Multiplicador de "sensação" de giro
	var turn_dir = input.steering
	
	# 1. GIRO NO PRÓPRIO EIXO (Baixa Velocidade)
	if is_on_ground and speed_kmh < SPEED_MIN_ASSIST and abs(input.steering) > 0.1:
	...
	# 2. AUXÍLIO DE CURVA DINÂMICO (Gradiente Progressivo)
...
func _handle_air_control(delta):
	...
	# Pitch (Invertido conforme pedido) e Yaw
...
func _handle_auto_flip(delta, speed_kmh):
	...
	if up_dot < 0.7 and speed_kmh < 15.0:
		flipped_timer += delta
		if flipped_timer > 0.4:
