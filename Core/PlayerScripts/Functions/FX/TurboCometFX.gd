...
func burst_fire_sequenced(total_size_multiplier: float) -> void:
	if _cached_markers.is_empty(): return
	...
	for marker in _cached_markers:
		if is_instance_valid(marker):
			...
			if flip_marker_direction: dir = -dir
...
func _jitter_direction(base_dir: Vector3, cone_degrees: float) -> Vector3:
	...
	if right.length_squared() < 0.0001: right = base_dir.cross(Vector3.RIGHT)
...
func _build_shared_mesh() -> void:
	...
	for i in range(streak_segments):
		...
		var col: Color
		if t_mid < 0.5: col = color_hot.lerp(color_mid, t_mid * 2.0)
		else: col = color_mid.lerp(color_tip, (t_mid - 0.5) * 2.0)
	...
	mat.blend_mode = BaseMaterial3D.BLEND_MODE_ADD # CORREÇÃO: Faltava o Blend ADD!
