...
func _handle_global_mouse(delta):
	...
	if mouse_vel.length() > 5.0: # Se houver qualquer movimento
		...
		look_vector.x += mouse_vel.x * camera_sensitivity
		look_vector.y -= mouse_vel.y * camera_sensitivity
		...
		# Ele assume 1.0 ou -1.0 baseada apenas no sinal (direção) da velocidade
