...
func fire_hitscan(shooter_node: Node3D, damage: float):
	...
	# =========================================================================
	# DETECÇÃO "GROSSA": uma cápsula alinhada com a direção do tiro, cobrindo
	# o trajeto inteiro de uma vez, em vez de um raio de espessura zero.
	# Continua sendo UMA query só — não fica mais pesado que antes na prática.
	# =========================================================================
	var capsule := CapsuleShape3D.new()
	capsule.radius = collision_radius
	capsule.height = max_range
	
	var mid_point: Vector3 = origin + forward_dir * (max_range * 0.5)
	...
	for c in candidates:
		...
		# intersect_shape não devolve ponto de impacto, então aproximamos a
		# distância projetando a posição do collider sobre a direção do tiro.
		var to_col: Vector3 = col.global_position - origin
		var dist_along: float = to_col.dot(forward_dir)
		if dist_along < 0.0 or dist_along > max_range: continue
		
		if dist_along < closest_dist:
			closest_dist = dist_along
			hit_collider = col
	
	var impact_point: Vector3 = (origin + forward_dir * closest_dist) if hit_collider else end_point
