...
func _handle_delivery_rules():
	...
	for pointer in get_tree().get_nodes_in_group("mission_pointers"):
		...
		if pointer.pointer_type == "dropoff":
			...
			# Setas gerais no delivery também respeitam a lista do portal
...
func _handle_generic_rules():
	...
	for pointer in get_tree().get_nodes_in_group("mission_pointers"):
		...
		if pointer.pointer_type == "dropoff":
			...
			# A MÁGICA: Só aparece se a seta (ou o pai dela) estiver no array do Portal!
			pointer.visible = _is_node_linked(pointer, active_nodes)
...
# ====================================================================
# HELPERS: FILTRAGEM INTELIGENTE PELO PORTAL
# ====================================================================

# 1. Pega os NodePaths do portal ativo e converte em instâncias reais de Node
...
# 2. Checa se a seta pertence aos nodes do portal
func _is_node_linked(pointer: Node, active_nodes: Array[Node]) -> bool:
	...
	# Sobe na árvore (Parent) verificando se a seta, ou o objeto em que ela está anexada,
	# faz parte da lista de coisas que o portal ativou.
...
# ====================================================================
# HELPER: Controle Geral de Limpeza
# ====================================================================
